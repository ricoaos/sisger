...
class Zend_Service_SqlAzure_Management_Client
{
    ...
    public function createServer($administratorLogin, $administratorPassword, $location)
    {
        ...
        $response = $this->_performRequest(self::OP_SERVERS, '', Zend_Http_Client::POST, array(
        ...
        if ($response->isSuccessful()) {
            ...
            return new Zend_Service_SqlAzure_Management_ServerInstance((string) $xml, $administratorLogin, $location);
        } else {
            require_once 'Zend/Service/SqlAzure/Management/Exception.php';
        ...
        if ($response->isSuccessful()) {
            ...
            if (! is_null($xmlServices)) {
                ...
                for ($i = 0; $i < count($xmlServices); $i ++) {
                    $services[] = new Zend_Service_SqlAzure_Management_ServerInstance((string) $xmlServices[$i]->Name, (string) $xmlServices[$i]->AdministratorLogin, (string) $xmlServices[$i]->Location);
                }
            }
            return $services;
        } else {
    ...
    public function setAdministratorPassword($serverName, $administratorPassword)
    {
        ...
        $response = $this->_performRequest(self::OP_SERVERS . '/' . $serverName, '?op=ResetPassword', Zend_Http_Client::POST, array(
            'Content-Type' => 'application/xml; charset=utf-8'
    ...
    public function createFirewallRule($serverName, $ruleName, $startIpAddress, $endIpAddress)
    {
        ...
        $response = $this->_performRequest(self::OP_SERVERS . '/' . $serverName . '/' . self::OP_FIREWALLRULES . '/' . $ruleName, '', Zend_Http_Client::PUT, array(
            'Content-Type' => 'application/xml; charset=utf-8'
        ...
        if ($response->isSuccessful()) {
            ...
            return new Zend_Service_SqlAzure_Management_FirewallRuleInstance($ruleName, $startIpAddress, $endIpAddress);
        } else {
            require_once 'Zend/Service/SqlAzure/Management/Exception.php';
            throw new Zend_Service_SqlAzure_Management_Exception($this->_getErrorMessage($response, 'Resource could not be accessed.'));
        }
    ...
    public function listFirewallRules($serverName)
    {
        ...
        if ($response->isSuccessful()) {
            ...
            if (! is_null($xmlServices)) {
                ...
                for ($i = 0; $i < count($xmlServices); $i ++) {
                    $services[] = new Zend_Service_SqlAzure_Management_FirewallRuleInstance((string) $xmlServices[$i]->Name, (string) $xmlServices[$i]->StartIpAddress, (string) $xmlServices[$i]->EndIpAddress);
                }
            }
            return $services;
        } else {
    ...
    public function deleteFirewallRule($serverName, $ruleName)
    {
        ...
        $response = $this->_performRequest(self::OP_SERVERS . '/' . $serverName . '/' . self::OP_FIREWALLRULES . '/' . $ruleName, '', Zend_Http_Client::DELETE);
