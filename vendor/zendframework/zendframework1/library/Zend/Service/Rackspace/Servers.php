...
class Zend_Service_Rackspace_Servers extends Zend_Service_Rackspace_Abstract
{
    ...
    public function createServer(array $data, $metadata = array(), $files = array())
    {
        ...
        $result = $this->httpCall($this->getManagementUrl() . '/servers', 'POST', null, null, json_encode(array(
            'server' => $data
        )));
        $status = $result->getStatus();
        switch ($status) {
    ...
    protected function updateServer($id, $name = null, $password = null)
    {
        ...
        $result = $this->httpCall($this->getManagementUrl() . '/servers/' . rawurlencode($id), 'PUT', null, null, json_encode(array(
            'server' => $data
        )));
        $status = $result->getStatus();
        switch ($status) {
    ...
    public function shareIpAddress($id, $ip, $groupId, $configure = true)
    {
        ...
        $result = $this->httpCall($this->getManagementUrl() . '/servers/' . rawurlencode($id) . '/ips/public/' . rawurlencode($ip), 'PUT', null, null, json_encode(array(
            'shareIp' => $data
        )));
        $status = $result->getStatus();
        switch ($status) {
    ...
    public function rebootServer($id, $hard = false)
    {
        ...
        $result = $this->httpCall($this->getManagementUrl() . '/servers/' . rawurlencode($id) . '/action', 'POST', null, null, json_encode($data));
        $status = $result->getStatus();
        switch ($status) {
            case '200':
            case '202': // break intentionally omitted
    ...
    public function createImage($serverId, $name)
    {
        ...
        $result = $this->httpCall($this->getManagementUrl() . '/images', 'POST', null, null, json_encode($data));
        $status = $result->getStatus();
        switch ($status) {
            case '202': // break intentionally omitted
                $image = json_decode($result->getBody(), true);
    ...
    public function changeBackupSchedule($id, $weekly, $daily)
    {
        ...
        $result = $this->httpCall($this->getManagementUrl() . '/servers/' . rawurlencode($id) . '/backup_schedule', 'POST', null, null, json_encode($data));
        $status = $result->getStatus();
        switch ($status) {
            case '204': // break intentionally omitted
                return true;
    ...
    public function disableBackupSchedule($id)
    {
        ...
        $result = $this->httpCall($this->getManagementUrl() . '/servers/' . rawurlencode($id) . '/backup_schedule', 'DELETE');
        $status = $result->getStatus();
        switch ($status) {
            case '204': // break intentionally omitted
                return true;
    ...
    public function createSharedIpGroup($name, $serverId)
    {
        ...
        $result = $this->httpCall($this->getManagementUrl() . '/shared_ip_groups', 'POST', null, null, json_encode($data));
        $status = $result->getStatus();
        switch ($status) {
