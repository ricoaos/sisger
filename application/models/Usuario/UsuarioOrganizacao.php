...
class Model_Usuario_UsuarioOrganizacao extends Zend_Db_Table
{
    ...
    public function getPerfilByParams($user, $grupo)
    {
        ...
        $select = $mUserOrg->select()
            ->setIntegrityCheck(false)
            ->distinct()
            ->from(array(
            'usorg' => 'usuario.usuario_organizacao'
        ), array(
            "usorg.*"
        ))
            ->join(array(
            'org' => 'organizacao.organizacao'
        ), 'org.id_organizacao = usorg.id_organizacao', array(
            "org.st_nome_fantasia"
        ))
            ->join(array(
            'per' => 'usuario.perfil'
        ), 'per.id_perfil = usorg.id_perfil', array(
            "per.st_nome"
        ))
