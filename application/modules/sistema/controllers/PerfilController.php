...
class Sistema_PerfilController extends App_Controller_Action
{
	public function init()
	{
		...
		$this->mSistema = new Model_Sistema_Menu();
	}
	
    public function indexAction()
    {      

        $select = $this->mSistema->select()
                  ->where('st_cancelado !=?', 'S')
                  ->order('num_ordem ASC');
        $rsMenu = $this->mSistema->fetchAll($select)->toArray();
        $this->view->rsMenu = $rsMenu;
    ...
    public function listagemAction()
    {
        ...
        $select = $mPerfil->select()
                 ->where('id_perfil !=?',1)
                 ->order('id_perfil ASC');
        $rsPerfil = $mPerfil->fetchAll($select)->toArray();
        $this->view->rsPerfil = $rsPerfil;
    }
